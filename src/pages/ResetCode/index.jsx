...
const ResetCode = () => {
  ...
  return (
    <div className="code" style={{backgroundImage: `url(${loginBg})`, backgroundSize: 'cover'}}>
      <div className="code__box">
        ...
        <form onSubmit={(e) => handleSubmit(e)}>
          <div className="code__items">
            {codeInputs.map(input => (
              <input
                ...
                onChange={e => hanldeCodeChange(e.target.value, input.inputIndex)}
                ref={ref => (codeInputRefs.current[input.inputIndex] = ref)}
              />
            ))}
          </div>
          ...
          <button className="code__submit" type="submit">continue</button>
        </form>
        <div className="code__resend">
          <p>Se você não recebeu clique aqui!</p>
          <button onClick={(e) => handleCodeResend(e)}>Reenviar</button>
        </div>
      </div>
    </div>
  )
